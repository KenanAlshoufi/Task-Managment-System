...
namespace Task_Managment__Windows_Forms_App_
{
    public partial class frmMyTasks : Form
    {
        ...
        public frmMyTasks(DTOUser UserInfo)
        {
            InitializeComponent();
            _UserInfo = UserInfo;
        }
        ...
        private async Task LoadAllCompletedTask()
        {
            try
            {
                var Response = await APIClient.TasksAPI.GetAsync($"GetAllTaskForUserIDCompleted/{_UserInfo.UserId}");
        ...
        private async Task LoadAllCancelledTask()
        {
            try
            {
                var Response = await APIClient.TasksAPI.GetAsync($"GetAllTaskForUserIDCancelled/{_UserInfo.UserId}");
        ...
        private async void frmMyTasks_Load(object sender, EventArgs e)
        {
            await LoadMyTask();
        ...
        private void frmMyTasks_MouseDown(object sender, MouseEventArgs e)
        {
            ReleaseCapture();
